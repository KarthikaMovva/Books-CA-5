...
const clickedregister = (storage) => {
  console.log(storage);
}; //Sending the input to console

const crossckeck = (stored) => {
  ...
  if (stored.Name.length === 0) {
    ...
    error.Name =
      "Number of letters in name should not be greater than 30 or less than 3";
  }
  ...
  if (stored.Password.length === 0) {
    ...
    const includingspecialchar = array.some((each) =>
      stored.Password.includes(each)
    );
  ...
  return (
    <div>
      ...
      <form onSubmit={formvalue.handleSubmit} className="form">
        {finalclick &&
        Object.keys(formvalue.touched).length === 4 &&
        Object.keys(formvalue.errors).length === 0 ? (
          <div className="final">Registration Successful!</div>
        ) : (
          <div> </div>
        )}
        <div>
          ...
          />
          {formvalue.touched.Name && formvalue.errors.Name ? (
            <p className="errors">{formvalue.errors.Name}</p>
          ) : null}
        </div>
        <div>
          <input
            ...
            onBlur={formvalue.handleBlur}
          />
          {formvalue.touched.Email && formvalue.errors.Email ? (
            <p className="errors">{formvalue.errors.Email}</p>
          ) : null}
        </div>
        <div>
          <input
            ...
            onBlur={formvalue.handleBlur}
          />
          {formvalue.touched.Password && formvalue.errors.Password ? (
            <p className="errors">{formvalue.errors.Password}</p>
          ) : null}
        </div>
        <div>
          <input
            ...
            value={formvalue.values.RepeatePassword}
            onBlur={formvalue.handleBlur}
          />
          {formvalue.touched.RepeatePassword &&
          formvalue.errors.RepeatePassword ? (
            <p className="errors">{formvalue.errors.RepeatePassword}</p>
          ) : null}
        </div>
        <button onClick={recordclick} type="submit" className="signup">
          Sign Up
        </button>
      </form>
    </div>
